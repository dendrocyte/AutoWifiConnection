 ...
 * @params
 */
class WiFiSwitchService_DeleteOriWifiConfig : WiFiSwitchService() {
    ...
    override fun tryConnectTarget() {
        ...
        if (nowSSID == targetQuotedSSID){
            ...
            //step5: get netId
            var oriConfig : WifiConfiguration? = null
            ...
            //step6: delete current network config
            //result always is false
            println("key: ${oriConfig?.preSharedKey}")
            oriConfig?.preSharedKey=""
            val isSuccess = manager.updateNetwork(oriConfig)
            println("update ori wifi: ${isSuccess != -1}")
